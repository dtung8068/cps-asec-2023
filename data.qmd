...
```{r}
df <- get_asec(year = 2023, vars = c("PERIDNUM", "A_FNLWGT", "GESTFIPS", "GTCO", "HEA", "A_AGE", "A_SEX", "PRCITSHP","PRDTRACE", "PEMLR","A_HGA","PEARNVAL","DIS_HP","NOW_COV", "MOOP"), key="033d6fdb469d970444d91cda781be61fa0d46099", tibble = FALSE)
```
...
sample <- df %>%
  filter(Age >= 18 & Empt == "Employed" & Earn < quantile(Earn, 0.95) & MExp < quantile(MExp, 0.95) ) %>%
  filter(Empt == "Employed") %>%
  slice_sample(prop = 0.01)

ggplot(sample, aes(x=Earn, y=MExp, label=State)) +
  geom_point(aes(color=Health), alpha=0.5) +
  scale_color_manual(values=c("Excellent" = "darkgreen", "Very good" = "lightgreen", "Good" = "grey", "Fair" = "orange", "Poor" = "red"))
```
